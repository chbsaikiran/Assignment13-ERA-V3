...
def generate_text(
    ...
    with torch.no_grad():
        for _ in range(max_length):
            seq_len = input_ids.size(1)  # Get the sequence length from input_ids
            attention_mask = torch.ones(input_ids.size(0), seq_len).to(device)
            position_ids = torch.arange(seq_len).unsqueeze(0).repeat(input_ids.size(0), 1).to(device)
